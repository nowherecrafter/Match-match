...
export function loadSettings(): void {
  ...
  // Загружаем текущие настройки из IndexedDB
  db.init().then(() => {
    db.getGameSettings().then((storedSettings) => {
                ...
                <div class="mb-3">
                  ...
                  <select id="${setting.name}" class="form-select">
                    ${setting.options.map(option => `
                      <option value="${option}" ${storedSettings && storedSettings[setting.name.toLowerCase()] === option ? 'selected' : ''}>
                        ${option}
                      </option>
                    `).join('')}
                  </select>
